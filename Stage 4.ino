...
void distCal(double dist) {
  ...
  if (XYdicration == true) {
    ...
    count = dist*78.5; //Don't ask, it just works // 0,785 er en værdi der ganges med fordi det virker
...
void turn(int angle){
    int f = 1;
    if(angle < 0) f = -1; //factor for inverting
    while(turnAngleDegrees < angle){ //checks angle differnce
        motors.setSpeeds(f*-111,f*100);
        turnSensorUpdate();
    }
    motors.setSpeeds(0,0);
